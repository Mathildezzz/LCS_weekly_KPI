...
SELECT DISTINCT member_detail_id, eff_reg_channel
           ...
           WHERE 1=1
             AND DATE(join_time) >= '{}'            -- start_date
             AND DATE(join_time) < '{}'             -- end_date
             AND UPPER(eff_reg_channel) LIKE '{}'   -- distributor_name
),
...
existing_2024_initial_purchase_base AS (
 ...
 WHERE DATE(order_paid_time) >= '2024-01-01' -- fixed
   AND DATE(order_paid_time) < '{}'          -- start_date
   AND rk = 1
),
...
LEFT JOIN new_member_base
 ...
 WHERE DATE(order_paid_time) >= '{}'            -- start_date
   AND DATE(order_paid_time) < '{}'             -- end_date
   AND UPPER(all_purchase_rk.eff_reg_channel) NOT LIKE '{}'  -- distributor_name 
   AND rk = 1
...
SELECT crm_member_id,
       MAX(CASE WHEN order_paid_time >= '{}' - 90 AND order_paid_time < '{}'  THEN 1 ELSE 0 END) AS if_3_month_not_purchased -- [-90, 0)
 FROM all_purchase -- 已过滤distributor
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}' - 90 AND order_paid_time < '{}' THEN 1 ELSE 0 END) = 1   -- start_date
),

existing_3_month_not_purchased_base AS (
SELECT crm_member_id,
       MAX(CASE WHEN order_paid_time >= '{}' - 90 AND order_paid_time < '{}' THEN 1 ELSE 0 END) AS if_3_month_not_purchased, -- [-90, 0)
       MAX(CASE WHEN order_paid_time >= '{}' - 180 AND order_paid_time < '{}' - 90 THEN 1 ELSE 0 END) AS if_3_6_month_purchased  -- [-180, -90)
 FROM all_purchase -- 已过滤distributor
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}' - 90 AND order_paid_time < '{}'  THEN 1 ELSE 0 END) = 0
    AND MAX(CASE WHEN order_paid_time >= '{}' - 180 AND order_paid_time < '{}' - 90 THEN 1 ELSE 0 END) = 1
),
...
SELECT crm_member_id,
       MAX(CASE WHEN order_paid_time >= '{}'  - 180 AND order_paid_time < '{}' THEN 1 ELSE 0 END) AS if_6_month_not_purchased, -- [-180, -90)
       MAX(CASE WHEN order_paid_time >= '{}'  - 270 AND order_paid_time < '{}' - 180 THEN 1 ELSE 0 END) AS if_6_9_month_purchased  -- [-270, -180)
 FROM all_purchase -- 已过滤distributor
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}' - 180 AND order_paid_time < '{}'  THEN 1 ELSE 0 END) = 0
    AND MAX(CASE WHEN order_paid_time >= '{}'  - 270 AND order_paid_time < '{}' - 180 THEN 1 ELSE 0 END) = 1
),


existing_9_month_not_purchased_base AS (
SELECT crm_member_id,
       MAX(CASE WHEN order_paid_time >= '{}' - 270 AND order_paid_time < '{}' THEN 1 ELSE 0 END) AS if_9_month_not_purchased, -- [-270, -0)
       MAX(CASE WHEN order_paid_time >= '{}' - 360 AND order_paid_time < '{}' - 270 THEN 1 ELSE 0 END) AS if_9_12_month_purchased  -- [-360, -270)
 FROM all_purchase -- 已过滤distributor
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}'- 270 AND order_paid_time < '{}' THEN 1 ELSE 0 END) = 0
    AND MAX(CASE WHEN order_paid_time >= '{}' - 360 AND order_paid_time < '{}' - 270 THEN 1 ELSE 0 END) = 1
),


existing_12_month_and_longer_not_purchased_base AS (
SELECT crm_member_id,
       MAX(CASE WHEN order_paid_time >= '{}' - 360 AND order_paid_time < '{}' THEN 1 ELSE 0 END)      AS if_12_month_not_purchased, -- [-360, -0)
       MAX(CASE WHEN order_paid_time < '{}' - 360 THEN 1 ELSE 0 END)                                             AS if_before_12_month_purchased  -- [-360, -270)
 FROM all_purchase -- 已过滤distributor
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}' - 360 AND order_paid_time < '{}' THEN 1 ELSE 0 END) = 0
    AND MAX(CASE WHEN order_paid_time < '{}' - 360 THEN 1 ELSE 0 END)  = 1
),
...
have_not_retained AS (
  SELECT DISTINCT retain_base.crm_member_id
        ...
        LEFT JOIN (SELECT DISTINCT crm_member_id FROM all_purchase WHERE order_paid_time >= '2024-01-01' AND order_paid_time < '{}') retained -- end_date
...
have_not_reactivated AS (
  SELECT DISTINCT trans.crm_member_id, distributor_name
        ...
        LEFT JOIN (SELECT DISTINCT crm_member_id::integer FROM existing_retain_base) retain
               ON trans.crm_member_id::integer = retain.crm_member_id::integer
          LEFT JOIN (SELECT DISTINCT crm_member_id FROM all_purchase WHERE order_paid_time >= '2024-01-01' AND order_paid_time < '{}') reactivated -- end_date
...
SELECT crm_member_id,
 ...
 GROUP BY 1
 HAVING MAX(CASE WHEN order_paid_time >= '{}' - 90 AND order_paid_time < '{}' THEN 1 ELSE 0 END) = 0
    AND MAX(CASE WHEN order_paid_time >= '{}' - 180 AND order_paid_time < '{}' - 90 THEN 1 ELSE 0 END) = 1        -- end_date
),
...
traffic_reg AS (
        SELECT '1 - new_member'::text                        AS member_type,
          ...
          WHERE agg_type = 'LEGO'
            AND CAST(traffic_table.date_id AS date) >= '{}'   -- start_date 
            AND CAST(traffic_table.date_id AS date) < '{}'   -- end_date
            AND distributor LIKE '{}'                       -- distributor_name
),
